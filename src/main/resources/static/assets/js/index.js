layui.config({
    ...
    $('#open').click(function () {
        var url = mUrl;
        window.open('https://pan.zhangyou710.cn/preview/onlinePreview?url=' + encodeURIComponent(Base64.encode(url)));
    });
    ...
    $('#btnDilaog').click(function () {
        layer.open({
            ...
            resize: false,
                ...
                if (typeof (mFilesUrls) !== "undefined" && mFilesUrls.length > 0) {
                    layer.msg('你选择了：' + JSON.stringify(mFilesUrls), {icon: 1});
                    mFilesUrls = undefined;
                }
            }
        });
    });
